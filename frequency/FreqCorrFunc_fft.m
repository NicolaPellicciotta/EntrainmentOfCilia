function [r_bin,N,cf,f_res]= FreqCorrFunc_fft(freq,f_filter, BoxSize)

        %%% calculate the correlation function from a frequency map
        %%% obtained with fft, freq is in 2d
        %%% filter is a single array
%        BoxSize=4;
        ...
        if nargin < 2 || isempty(f_filter)
         ...
         %R_bin= BoxSize:BoxSize*3:;
         R_bin=logspace(0,2,10)*4;
