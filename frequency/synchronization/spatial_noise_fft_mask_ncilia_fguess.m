function [Meta] = spatial_noise_fft_mask_ncilia_fguess(box_size,movie,BW,mask_input)
% 
if nargin < 2 || isempty(movie)
    movie='P0';
end

if nargin < 4 || isempty(mask_input)
    mask_input=[];
end
...
[fq,m_pxx] = average_fft(fs,BW,FR,2);
figure()
plot(fq,m_pxx); xlim([15,25]);
prompt= 'fguess';
f_guess = (input(prompt));
fq_max=f_guess+7;fq_min=f_guess-7;
close all

Meta.f_guess=f_guess; 
...
for xx=1:(size(X,2)-1);
    for yy=1:(size(X,1)-1);    
        ...
        if ~isempty(mask_input)
             good= all(mask_input(Y(yy,xx):(Y(yy+1,xx+1)-1),X(yy,xx):(X(yy+1,xx+1)-1)));l   
        else
