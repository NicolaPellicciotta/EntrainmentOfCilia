...
clear all;
C0_array=[0.67*10^(-6),...  %%%%% value from the centrin area model C=a/sqrt(pi)
    0.11*10^(-6)];          %%%% value for the limit of touching cilia   
...
%        leg{cc}=num2str(nc); 
        if Cell(nc).eps(v_eps)<10;
            EPS_V(cc)=Cell(nc).eps(v_eps)/(Flow_mean(v_eps));
            EPS(dd)=Cell(nc).eps(v_eps);
        else EPS_V(cc)=nan; EPS(dd)=nan;
        
        end
        NCILIA_ALL(cc)=Cell(nc).Ncilia;
        VOLTAGE(cc)=jj;
        NCILIA(dd)= Cell(nc).Ncilia;
        cc=cc+1;
        dd=dd+1;
        end
...
set(gcf,'position',[x0,y0,width,height])
set(gca,'FontSize',15);
plot(1:30,fit_out_all1(1:30),'r-','MarkerSize',mks,'LineWidth',lw+2);
plot(1:30,fit_out_all(1:30),'g-','MarkerSize',mks,'LineWidth',lw+2);
...
fig=figure(15);
%saveas(fig,'/u/homes/np451/Desktop/sync_paper/figures//suppl_epsilon_v_n.pdf')
saveas(fig,'/u/homes/np451/Dropbox/Synchonisation of mammalian cilia with hydrodynamic forces/PNAS/figures/v3/supp/epslion_v_alldata/suppl_epsilon_v_n_a_1.2um_log.pdf')
